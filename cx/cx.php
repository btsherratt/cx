...
function cx($db_path, $data_folder_path, $public_data_folder_path) {
	...
	if (cx_setup_required()) {
		...
		include('secrets.php');
		cx_users_add_user(CX_SETUP_USER, CX_SETUP_PASSWORD, CX_SETUP_URL);
		cx_sites_add_site(1);
		//cx_posts_add_post(1, 'Wow, it\'s finally here!', null, 'Finally got this website to work!!');
		exit;
	}
			...
			if (cx_admin_logged_in() == false) {
				...
				if (count($path_components) >= 2 && $path_components[1] == 'logout') {
					...
					if (isset($_GET['id']) == false or $_GET['id'] == 0) {
						cx_posts_add_post(1, $title, $slug, $data);
					} else {
						$id = $_GET['id'];
						cx_posts_update_post($id, $title, $slug, $data);
					}
